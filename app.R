...
coralcoverS <- read.csv(here("data","coralcoverS.csv"))
...
#Mote sites
triangleSites<- read.csv(here("data","Mote_sites.csv"))
MoteCover<- read.csv(here("data","Mote_cover.csv"))
TravisRates<- read.csv(here("data","Travis_rates.csv"))
Bioerosion<-read.csv(here("data","Bioerosion.csv"))
#merge all data frame
data<-merge(data,coralcoverS,by=c("Time","variable","Scenario"))
...
#recreate data_frame
df<-data.frame(num,ncc,sdncc,RAP,sdRAP,AH,sdAH,coralcover,adaptation,scenario,Site,lat,long,SLR,accSLR,AH10,Coral,Macro,Micro,CCA)
             ...
             tabPanel("Home",
                      div(class="outer",
                          ...
                          absolutePanel(id = "controls", class = "panel panel-default",
                                        ...
                                        hidden(div(id = "more_info_text", class = "hidden-text", "Carbonate budget projections are used as a metric for reef persistence. A carbonate budget 
                                  ...
                                  into the future using site-specific climatic projections and species-specific relationships between rates of calcification and erosion and ocean acidification and temperature.",style="color:#045a8d"))
                          ...
                          absolutePanel(id = "controls", class = "panel panel-default",
                                        ...
                                        sliderInput("home_adaptation", label = h5("Change bleaching tolerance (in \u00B0C)"),
                                                    min = 0,  max = 2, step=0.25,value=0),
                                        
                                        
                                        radioButtons("home_scenario",label = h4("Choose emission scenario"),
                                                     choices = list("Reduce gas emission" = "SSP2_4.5", "Business as usual" = "SSP5_8.5"),
                                                     selected = "SSP5_8.5",
                                                     inline = F,
                                                     width = "100%"),
                                        actionButton("more_info2", "More info", icon = icon("info")),
                                        hidden(div(id = "more_info2_text", class = "hidden-text", "The two scenarios presented here refer to SSP5-8.5 (Buisness as usual) which is the pathway that represents current rates of emissions and emissions growth. It is considered a 
...
ambitious but still possible scenario. It is considered a “middle of the road” pathway whith intermediate CO2 emissions peaking in 2040 and gradually declining towards 2100." ,style="color:#045a8d"))
             ...
             tabPanel("Reef Characteristics", value = "reef",
                      tags$style("
        #controls {
      ...
      ")),
                                     ...
                                     radioButtons("reef_scenario",label = h4("Let's go green"),
                        ...
                        column(3,
                               ...
                               box(width = NULL,align='center', collapsible = F, title = tagList(
                               ...
                               ),background='navy',solidHeader = F,
                               collapsed = F,imageOutput("myImage",width="22vw",height="auto")
                               ),
                               box(width =NULL, title = tagList(
                                 div("Sea level rise projections", style = "font-size: 24px; font-weight: bold;")
                                 
                               ),collapsible = F,background='navy',
                               collapsed = F, 
             ...
             tabPanel("Restoration & Adaptation", value = "reef2",
                      tags$style("
        #controls {
          background-color: white;
          opacity: 0.6;
      ...
      ")),
                                     ...
                                     sliderTextInput("plot_date3",
                                                     ...
                                                     grid = FALSE,
                                                     animate=animationOptions(interval = 3000, loop = FALSE)),
                                     radioButtons("reef_scenario2",label = h4("Let's go green"),
                                                  choices = list("Reduce gas emissions" = "SSP2_4.5", "Business as usual" = "SSP5_8.5"),
                                                  selected = "SSP5_8.5",
                                                  inline = F,
                                                  width = "100%")
                          ...
                          box(
                            ...
                            fluidRow(
                              ...
                              column(4,
                                     ...
                                     div(style = "text-align: center;",
                                         ...
                                         hidden(div(id = "Branching_info_text", class = "hidden-text", "Branching and plating corals grow rapidly, forming large, 
                            tree-like colonies that provide structural complexity, effectively shading out competitors for light. These corals are highly susceptible to breakage during storms and suffer high mortality rates 
                                       following temperature anomalies. This sensitivity means they can only be dominant in ideal environments." ,style="color:#045a8d")))
                              ),
                              ...
                              column(4,
                                     ...
                                     div(style = "text-align: center;",
                                         ...
                                         hidden(div(id = "Builders_info_text", class = "hidden-text", "Domed colonies with moderate growth rates that can reach large colony sizes. 
                                This group includes corals such as Orbicella spp., Montastrea cavernosa and brain corals. These reef builders represent a generalist stress tolerant 
                                           life-history strategy that can do well in habitats where competition is limited by low levels of stress.", style = "color:#045a8d"))
                                     )
                              ),
                              ...
                              column(4,
                                     ...
                                     div(style = "text-align: center;",
                                         ...
                                         hidden(div(id = "Weedy_info_text", class = "hidden-text", "Small corals with brooding reproduction, fast growth rates, high population turnover 
                            that can opportunistically colonise recently disturbed habitats. ‘Weedy’ species include Porites astreoides and Siderastrea spp. 
                                       are more likely to be ‘winners’ and persist in unfavourable and disturbed environments." ,style="color:#045a8d")))
                              )
                            )
                          ),
                          ...
                          box(
                            ...
                            fluidRow(
                              ...
                              column(12,
                                     ...
                                     sliderInput("home_adaptation2", label = NULL, post="\u00B0C",
                                                 min = 0, max = 2, step = 0.25, value = 0)
                              )),
                            div(style = "text-align: center;",
                                actionButton("Adaptation_info", "More info", icon = icon("info"),  style = "color: white; border: none;"),
                                hidden(div(id = "Adaptation_info_text", class = "hidden-text", "To explore the potential effects of coral thermal adaptation on reef 
                              persistence, the bleaching threshold of all corals is increased up to 2°C by increments of 0.25°C. 
                              By adding adaptation to your scenario, bleaching is delayed in time.   " ,style="color:#045a8d")))
                          ),
                          fluidRow(
             ...
             tabPanel("Mote Sites Restoration", value = "Planning Restoration",
                      sidebarLayout(
                        ...
                        sidebarPanel(
                          ...
                          pickerInput("selected_site", "Select Site", choices = unique(MoteCover$Site)),
                          br(),
                          ...
                          div(
                              ...
                              hidden(div(id = "Restoration_info_text", class = "hidden-text", "The corals accompanied by a shaded slider bar come from your selected site but aren’t candidates for restoration. 
                                         If a species that can be restored has a percentage above zero, that species already occurs at the site, and additional planting is possible." ,style="color:white")))
                        ),
                        ...
                        mainPanel(
                          ...
                          fluidRow(
                            column(width = 12, align = "left",
                                   tags$div(style = "display: inline-flex; gap: 10px; padding-top: 0px;",
                 ...
                 column(
                   ...
                   shinydashboard::box(
                     ...
                     selectizeInput(
                       "baseline_species",
                       "select your species:",
                       choices = sort(unique(MoteCover$Species)),   # assumes available in global
                       multiple = TRUE,
             ...
             tabPanel("Data",
                      ...
                      "Projections for Cheeca Rocks can be found ", tags$a(href="https://www.nature.com/articles/s41598-022-26930-4", 
                                                                           "here.")
             ),
             #----------------------------------ABOUT THIS SITE---------------------------------------------------               
...
numPal <- colorNumeric(c('darkred','red','#FFFF99','#CCFF99','#33CCCC','#0099FF','#0033FF','#000066'), domain = at)
# Define labels
labels <- c("Eroding", "", "", "","Growing")
#numPal <- colorBin("RdYlBu",  bins = at, domain = at)
...
server = function(input, output, session) {
  ...
  reefName <- reactiveVal('Cheeca Rocks')
  reefYear<-reactiveVal(2019)
  reefAdaptation<-reactiveVal(0)
  reefScenario<-reactiveVal('SSP5_8.5')
  initialBudget <- reactiveVal(NULL)
  ...
  output$chosenReef <- renderText({paste(
    input$selectReef, "reef looks like this:")
  })
  output$chosenReef2 <- renderText({paste(
  ...
  observeEvent(input$home_adaptation,{
    reefAdaptation(input$home_adaptation)
    updateSliderInput(session, "reef_adaptation", value=reefAdaptation())
  })
  observeEvent(input$plot_date,{
    
    reefYear(input$plot_date)
    updateSliderTextInput(session, "plot_date2", selected=reefYear())
    
  })
  observeEvent(input$home_scenario,{
    reefScenario(input$home_scenario)
    updateRadioButtons(session, "reef_scenario", selected=reefScenario())
  })
  # Observe changes in the second slider and update the first slider
  observeEvent(input$plot_date2, {
    reefYear(input$plot_date2 )
    
    updateSliderTextInput(session, "plot_date", selected = reefYear())
    
  })
  # Observe changes in the second set of radio buttons and update the first set
  observeEvent(input$reef_scenario, {
    reefScenario(input$reef_scenario) # Update the state with the second set's value
    updateRadioButtons(session, "home_scenario", selected = reefScenario())
  })
  #input slider from second tab changes Circle shematic
  observeEvent(input$reef_adaptation,{
    reefAdaptation(input$reef_adaptation)
    #  updateSliderInput(session, "reef_adaptation", value=reefAdaptation())
  })
  observeEvent(input$plot_date2,{
  ...
  observeEvent(input$reef_scenario,{
    reefScenario(input$reef_scenario)
    #  updateRadioButtons(session, "reef_scenario", selected=reefScenario())
  })
  ...
  filteredDF = reactive({
    df %>%
      filter(num %in%  reefYear()) %>%
      filter(scenario %in%  reefScenario())%>%
      filter(adaptation %in% reefAdaptation())
  ...
  output$mymap <- renderLeaflet({
    leaflet() %>%
      ...
      addLegendNumeric(
        pal = numPal <- colorNumeric(c('darkred','red','#FFFF99',
  ...
  observe({
    leafletProxy("mymap", data = filteredDF()) %>%
      clearShapes() %>%  # Clear previous circles
      addCircles(lng = ~long, lat = ~lat, weight = 40,
                 popup = ~paste(
  ...
  observe({
    leafletProxy("mymap", data = triangleSites) %>%
      addCircleMarkers(
        lng = ~long, lat = ~lat,
        radius = 9,
        weight = 5,
        color = "red",
        fillColor = ~numPal(ncc),
        fillOpacity = 0.8,
        stroke = TRUE,
        popup = ~paste(
  ...
  output$baseline_budget_box <- renderValueBox({
    ...
    coral_data <- MoteCover %>%
      filter(Site == input$selected_site, Class == "HC", !is.na(months12)) %>%
      select(Species, Cover = months12,Location)
    
    result <- coral_data %>%
      left_join(TravisRates, by = "Species") %>%
  ...
  output$baseline_RAP_box <- renderValueBox({
    ...
    BioSponges<- bio_vals$BioSponges
    # Micro available as bio_vals$Micro if you need it
    
    # RAP formula (mm/yr)
    rap_values$baseline <- (net_budget +((-PF * 0.25) - (Urchins + BioSponges) * 0.5)) / (2.89 * (1 - 0.558))
  ...
  output$restored_budget_box <- renderValueBox({
    ...
    if (total_cover > 100) {
      return(valueBox(
        value = tags$p("—", style = "font-size: 2.1vw;"
                       ),
    ...
    BioSponges<- ifelse(is.na(bio_vals$BioSponges), 0, bio_vals$BioSponges)
    Micro     <- ifelse(is.na(bio_vals$Micro), 0, bio_vals$Micro)
    
    erosion_total <- PF + Urchins + BioSponges + Micro
  ...
  BioSponges <- as.numeric(bio_vals$BioSponges)
  rap_values$restored <- (net_budget +((-PF * 0.25) - (Urchins + BioSponges) * 0.5)) / (2.89 * (1 - 0.558)) 
  ...
  output$stateBox <- renderValueBox({
    ...
    # Determine which reef to use: either the selected reef or the reefName() from tab one
    ...
    dat <- filteredDF() %>% filter(Site == reef_to_use)
  ...
  output$coverBox <- renderValueBox({
    dat <- filteredDF() %>% filter(Site==reefName())
    valueBox(value = tags$p(paste0( round(dat$coralcover), "%"), style = "font-size: 2vw;"),
             "Coral cover", icon = icon("chart-pie"),
  ...
  output$carbonateBox <- renderValueBox({
    dat <- filteredDF() %>% filter(Site==reefName())
    valueBox(value = tags$p(HTML(paste0(round(dat$ncc,1),"kg/m",tags$sup("2"),"/year")), style = "font-size: 2vw;"),
             "Carbonate budget", icon = icon("scale-balanced"),
             color = "light-blue",
  ...
  output$rapBox <- renderValueBox({
    dat <- filteredDF() %>% filter(Site==reefName())
    valueBox(value = tags$p(paste0( round(dat$RAP,1),"mm/year"), style = "font-size: 2vw;"),
             "Vertical reef growth", icon = icon("layer-group"),
             color = "blue",
  ...
  output$slrBox <- renderValueBox({
    dat <- filteredDF() %>% filter(Site==reefName())
    valueBox(value = tags$p(paste0( round(dat$SLR,1),"mm/year"), style = "font-size: 2vw;"),
             "Sea level rise", icon = icon("house-flood-water"),
             color = "teal",
  ...
  output$Carbonate <- renderText({
    ...
    paste("The net carbonate budget equals",
  ...
  output$Cover <- renderText({
    ...
    paste("Coral cover is ",
          round(dat$coralcover),"%.")
  })
  output$seaL <- renderText({
    dat <- filteredDF() %>% filter(Site==reefName())
    
    paste("Sea level is increasing by",
  ...
  output$slr_curve <- renderPlot({
    ggplot(filtered_slr(), aes(x = Time, y = cumsum(HR/10))) + # put a cumsum wrapper around y
      ...
      ylab("Sea level rise (mm)") + xlab("Year") + theme_bw() + 
  ...
  output$SLRmetrics <- renderText({
    reef_depth = filteredDF() %>%
      filter(Site==reefName()) %>%
      pull(reef_depth)
    
    if(reef_depth < 0) {
      paste0("The natural wall of the reef is now \n",
  ...
  output$myImageSLR <- renderImage({
    ...
    if(dat$reef_depth < 0) {
      ...
      p1<-    ggplot() +
        ...
        geom_ribbon(data= reef,
                    aes(x,ymin=y, ymax=dat$rdScaled),
                    fill='black', color = "black", linewidth=0.5) +
        geom_ribbon(data= reef1,
        ...
        annotation_raster(img, 
                          ...
                          ymin = dat$rdScaled - 1.5, # Adjust ymin to make it larger vertically
                          ymax = dat$rdScaled + 2.5) +
        theme_void() +
      ...
      p2<-   ggplot() +
        geom_circle(aes(x0=0,
                    ...
                    inherit.aes = F) +
        geom_ribbon(data= reef,
                    aes(x,ymin=dat$rdScaled, ymax=y),
                    fill="deepskyblue3", color = "white", linewidth=0.5) +#3ba3bf#43accb
        geom_ribbon(data= reef1,
                    aes(x1, ymin=(((dat$SLR-0.4576675)/(3.284522-0.4576675)) + 1.5 + 0.03 * sin(10 * x1)), ymax=y1),
                    fill="aliceblue", color="white", linewidth=0.5) +
        geom_segment(aes(x=-2.8, xend=2.8, y=0.95, yend=0.95), 
                     linetype='dotted', linewidth=0.8, color="#2c383e") +
        geom_segment(data= reef,aes(x=-x, xend=x, y=dat$rdScaled, yend=dat$rdScaled), 
  ...
  output$rapPlot <- renderPlot({
    ...
    p<-ggplot(filtered_data(), aes(x = Time, y = RAP)) + geom_line(colour="white") + geom_point(aes(size = ifelse(Time == input$plot_date3, 6, 3)),
                                                                                                alpha = 0.8, colour = "white") +
      ylab("vertical reef growth (mm/year)") + xlab("Year") + theme_bw() + 
      theme(legend.title = element_blank(), legend.position = "", plot.title = element_text(size=10), 
            plot.margin = margin(5, 12, 5, 5),
            axis.text = element_text(size=12,color = "white"),              # Set axis text color to white
            axis.title = element_text(size=14,color = "white"),  
  ...
  output$wSLRPlot <- renderPlot({
    ggplot(filtered_data(), aes(x = Time, y = RAP)) + geom_line(colour="white") + geom_point(aes(size = ifelse(Time == input$plot_date3, 6, 3)),
                                                                                             alpha = 0.8, colour = "white") +
      geom_line(aes(x=Time,y=HR/10),data=filtered_slr2(),colour="#00d4d4")+
      ylab("vertical reef growth & SLR (mm/year)") + xlab("Year") + theme_bw() + 
      theme(legend.title = element_blank(), legend.position = "", plot.title = element_text(size=10), 
            plot.margin = margin(5, 12, 5, 5),
            axis.text = element_text(size=12,color = "white"),              # Set axis text color to white
            axis.title = element_text(size=14,color = "white"),  
...
#----------------------------RESTORATION PLANNING 2----------------------------------------
  ...
  .safe_num <- function(x) { if (is.null(x) || is.na(x)) 0 else as.numeric(x) }
  ...
  output$baseline_budget <- shinydashboard::renderValueBox({
    ...
    ids <- paste0("base_", gsub("[^A-Za-z0-9]", "_", sp))
    covers <- sapply(ids, function(id) { v <- input[[id]]; if (is.null(v)) 0 else as.numeric(v) })
    
    # net production = sum( cover * rate / 100 ), matching rates from TravisRates
    rates <- as.numeric(TravisRates$rate[match(sp, TravisRates$Species)])
  ...
  output$restored_budget <- shinydashboard::renderValueBox({
    ...
    sp  <- if (is.null(input$baseline_species)) character(0) else input$baseline_species
    ids <- paste0("base_", gsub("[^A-Za-z0-9]", "_", sp))
    base_vals <- sapply(ids, function(id) { v <- input[[id]]; if (is.null(v)) 0 else as.numeric(v) })
    base_rates <- as.numeric(TravisRates$rate[match(sp, TravisRates$Species)])
    ...
    slider_ids <- paste0("rest_slider_", gsub("[^A-Za-z0-9]", "_", restoration_species))
    rest_vals <- sapply(slider_ids, function(id) { v <- input[[id]]; if (is.null(v)) 0 else as.numeric(v) })
    rest_rates <- as.numeric(TravisRates$rate[match(restoration_species, TravisRates$Species)])
