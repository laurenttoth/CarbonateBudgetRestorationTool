...
setwd("Data")
baseline_budgets <- read.csv("NCRMP_CarbonateBudgets_2014_to_2024.csv", header = TRUE)
...
baseline_budgets$rap_status <- ifelse(  # invalid: baseline_Budgets vs baseline_budgets mismatch in source
  baseline_budgets$rap < -0.5, "Eroding",
  ifelse(baseline_budgets$rap > 0.5, "Growing", "Stasis")
)
...
lm_cc_rap <- lm(baseline_budgets$reef_accretion_potential ~ baseline_budgets$percent_cover_hard_coral)

# summary(lm_cc_rap)
cc_rap_slope <- lm_cc_rap$coefficients[2]
cc_rap_intercept <- lm_cc_rap$coefficients[1]

baseline_budgets$restored_rap <- cc_rap_slope *
  (baseline_budgets$percent_cover_hard_coral + restored_cover) + cc_rap_intercept

# check to make sure we're not exceeding 100% cover
# table(baseline_budgets$percent_cover_hard_coral + restored_cover > 100) #ok at 30% max
